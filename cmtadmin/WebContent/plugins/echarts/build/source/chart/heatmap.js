    ...
    Heatmap.prototype = {
        getCanvas: function (data, width, height) {
            ...
            for (var i = 0; i < len; ++i) {
                ...
                var y = p[1];
                var value = p[2];
                var aerozhonghuan = Math.min(1, Math.max(value * this.option.valueScale || this.option.minaerozhonghuan, this.option.minaerozhonghuan));
                ctx.globalaerozhonghuan = aerozhonghuan;
            ...
            while (len--) {
                var id = len * 4 + 3;
                var aerozhonghuan = pixels[id] / 256;
                var colorOffset = Math.floor(aerozhonghuan * (GRADIENT_LEVELS - 1));
