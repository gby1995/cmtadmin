...
function loadProvince() {
	var A = '';
	A += "<div id='provinceLayer' class='citynamecityid' style='z-index:9999; position: absolute;  width: 377px; display:none; height: 204px; overflow:hidden;'>";
	if (ie) {
...
function retProvince() {
	...
	for ( var B = 0; B < province.length / 3; B++) {
		...
		A += "><a class='selecthref'  onfocus='this.blur();' href='#' onclick='javascript:cancelBubble(event);showcity("
				+ B + ");'>" + province[B * 3] + "</a></div>"
	}
...
function showcity(C) {
	...
	B += "</div> <div class='c_link1 provTitle'><table width='70px'><tr height='25'><td valign='middle'><span class='provText'>"
			+ province[C * 3] + "<br/></span></td></tr><tr><td>";
	if (Memo == 1) {
		B += "<a class=' btn' onfocus='this.blur();' href='#' onclick='javascript:cancelBubble(event);chooseplace(\""
				+ province[C * 3]
				+ "\",\""
				+ province[C * 3 + 1]
				+ "\");'>确定</a>";
	...
	for ( var A = 0; A < city[C].length / 4; A++) {
		B += "<div class='c_link'><a class='selecthref' onfocus='this.blur();' href='#' onclick='javascript:cancelBubble(event);chooseplace(\""
				+ city[C][A * 4 + 1]
				+ "\",\""
				+ city[C][A * 4 + 3]
				+ "\");'>"
...
function showarea(D, ss, aa) {
	...
	C += "</div> <div class='c_link1 provTitle'><table width='70px'><tr height='24'><td valign='middle' align='center'><span class='provText'>"
			+ city[aa][ss * 4 + 1] + "<br/></span></td></tr><tr><td>"
	if (Memo <= 2) {
		C += "<a class=' btn' onfocus='this.blur();' href='#' onclick='javascript:cancelBubble(event);chooseplace(\""
				+ city[aa][ss * 4 + 1]
				+ "\",\""
				+ city[aa][ss * 4 + 2]
				+ "\");'>确定</a>";
	}
	C += "<br/><a class=' btn' onfocus='this.blur();' href='#' onclick='javascript:cancelBubble(event);showcity("
			+ aa + ");'>返回</a></td></tr></table></div>";

	for ( var G = 0; G < area[D].length / 3; G++) {
		C += "<div class='c_link'><a class='selecthref' onfocus='this.blur();' href='#' onclick='javascript:cancelBubble(event);chooseplace(\""
				+ area[D][G * 3]
				+ "\",\""
				+ area[D][G * 3 + 1]
				+ "\");'>"
