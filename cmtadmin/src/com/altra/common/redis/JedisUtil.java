...
//			}
	        if (jedisPool == null) { 
	            ...
	            config.setMaxActive(Constants.REDIS_MAX_ACTIVE);   
	            config.setMaxIdle(Constants.REDIS_MAX_IDLE);
	            config.setMaxWait(Constants.REDIS_MAX_WAIT);
		...
		public class Strings {
			...
			public String get(String key,int... indexDB ) {
				...
				if(indexDB!=null && indexDB.length>0){
					sjedis.select(indexDB[0]);
				}
				String value = sjedis.get(SafeEncoder.encode(SafeEncoder.encode(key)));
				returnJedis(sjedis);
				return value;
			}
...
//				return set(SafeEncoder.encode(key), SafeEncoder.encode(value));
				Jedis jedis = getJedis();
				if(indexDB!=null && indexDB.length>0){
					jedis.select(indexDB[0]);
				}
				String status = jedis.set(SafeEncoder.encode(SafeEncoder.encode(key)), SafeEncoder.encode(SafeEncoder.encode(value)));
				returnJedis(jedis);
				return status;
			}
			...
			public long del(String key,int...indexDB){
				if(indexDB!=null && indexDB.length>0){
					return del(SafeEncoder.encode(key),indexDB[0]);
				}else{
			 ...
			 * @return
			 */
			public long del(byte[] key,int... indexDB){
				Jedis jedis = getJedis();
				if(indexDB!=null && indexDB.length>0){
					jedis.select(indexDB[0]);
				}
		...
		public class Lists {
			...
			public String rpop(String key) {
				Jedis jedis = getJedis();
				String value = jedis.rpop(SafeEncoder.encode(SafeEncoder.encode(key)));
				returnJedis(jedis);
				return value;
			}
			 ...
			 * @return 记录总数
			 * */
			public long lpush(String key, String value,int... indexDB) {
//				return lpush(SafeEncoder.encode(SafeEncoder.encode(key)), SafeEncoder.encode(SafeEncoder.encode(value)));

				Jedis jedis = getJedis();
				if(indexDB!=null && indexDB.length>0){
					jedis.select(indexDB[0]);
				}
				long count = jedis.lpush(SafeEncoder.encode(SafeEncoder.encode(key)), SafeEncoder.encode(SafeEncoder.encode(value)));
				returnJedis(jedis);
				return count;
			}
			...
			public long rpush(String key, String value) {
				Jedis jedis = getJedis();
				long count = jedis.rpush(SafeEncoder.encode(SafeEncoder.encode(key)), SafeEncoder.encode(SafeEncoder.encode(value)));
				returnJedis(jedis);
				return count;
			}
			...
			public List<String> lrange(String key, long start, long end,int... indexDB) {
				...
				List<String> list = sjedis.lrange(SafeEncoder.encode(SafeEncoder.encode(key)), start, end);
				returnJedis(sjedis);
				return list;
			}
			 ...
			 * @return 删除后的List中的记录数
			 * */
			public long lrem(String key, int c, String value,int... indexDB) {
//				return lrem(SafeEncoder.encode(key), c, SafeEncoder.encode(value));
				Jedis jedis = getJedis();
				if(indexDB!=null && indexDB.length>0){
					jedis.select(indexDB[0]);
				}
				long count = jedis.lrem(SafeEncoder.encode(SafeEncoder.encode(key)), c, SafeEncoder.encode(SafeEncoder.encode(value)));
				returnJedis(jedis);
				return count;
